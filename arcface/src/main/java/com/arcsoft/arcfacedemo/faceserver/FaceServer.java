...
public class FaceServer {
     ...
     * @return 是否初始化成功
     */
    public boolean initnew(Context context,String faceFeatrue) {
        synchronized (this) {
            if (faceEngine == null && context != null) {
                faceEngine = new FaceEngine();
                int engineCode = faceEngine.init(context, FaceEngine.ASF_DETECT_MODE_IMAGE, FaceEngine.ASF_OP_0_HIGHER_EXT, 16, 1, FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_FACE_DETECT);
                if (engineCode == ErrorInfo.MOK) {
                    initFaceListnew(context,faceFeatrue);
                    return true;
                } else {
                    faceEngine = null;
                    Log.e(TAG, "init: failed! code = " + engineCode);
     ...
     * @param context 上下文对象
     */
    private void initFaceListnew(Context context,String faceFeatrue) {
        synchronized (this) {

            faceRegisterInfoList = new ArrayList<>();
            faceRegisterInfoList.clear();
            String userName = "register" == null ? String.valueOf(System.currentTimeMillis()) : "register";
            faceRegisterInfoList.add(new FaceRegisterInfo(HexUtils.hex2Bytes(faceFeatrue), userName));
        ...
        synchronized (this) {
            if(faceEngine == null){
                if(context == null){
                    ...
                    ConfigUtil.setFtOrient(context, FaceEngine.ASF_OP_0_HIGHER_EXT);
                }
...
//               String s1 =HexUtils.bytes2Hex(faceFeature.getFeatureData());
//                PreferencesUtils.putString(context,"cardFeature",s1);
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        fosFeature.close();
//                        String str = Base64.encodeToString(faceFeature.getFeatureData(), Base64.DEFAULT);
//                        byte[]face=  Base64.decode(str, Base64.DEFAULT);
...
////                        faceFeature1.setFeatureData(face);
     ...
     * @return 是否注册成功
     */
    public boolean registenew(Context context, byte[] nv21, int width, int height, String name) {
        synchronized (this) {
            if(faceEngine == null){
                if(context == null){
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    //3.保存注册结果（注册图、特征数据）
                    if (code == ErrorInfo.MOK) {
                        sFeature =HexUtils.bytes2Hex(faceFeature.getFeatureData());
                        YuvImage yuvImage = new YuvImage(nv21, ImageFormat.NV21, width, height, null);
                        //为了美观，扩大rect截取注册图
                        Rect cropRect = getBestRect(width, height, faceInfoList.get(0).getRect());
                        if (cropRect == null) {
...
////                        faceFeature1.setFeatureData(face);
    ...
    /**
     * 如果有特征值，使用特征值注册人脸(单独拿出来)
     *
     * @param name    保存的名字，可为空
     * @return 是否注册成功
     */
    public boolean registenew1( String name,String faceFeatrue) {
        synchronized (this) {
            String userName = name == null ? String.valueOf(System.currentTimeMillis()) : name;
            if (faceRegisterInfoList == null) {
                faceRegisterInfoList = new ArrayList<>();
    ...
    /**
     * 在特征库中搜索(如果存在特征值的处理方法)
     *@param myfaceFeature 底片人脸特征值
     * @param faceFeature 传入特征数据
     * @return 比对结果
     */
    public CompareResult getTopOfFaceLibNew(Context context,FaceFeature faceFeature,String myfaceFeature) {
        if(faceEngine == null){
            if(context == null){
                return null;
            }else {
        ...
        float maxSimilar = 0;
            ...
            //底片人脸特征
        tempFaceFeature.setFeatureData(HexUtils.hex2Bytes(myfaceFeature));
        faceEngine.compareFaceFeature(faceFeature, tempFaceFeature, faceSimilar);
        if (faceSimilar.getScore() > maxSimilar) {
                maxSimilar = faceSimilar.getScore();
       ...
       return new CompareResult("", maxSimilar);
    ...
    public   int extractFaceFeature(Context context, byte[] nv21, int width, int height){
