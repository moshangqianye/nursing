package com.jqsoft.livebody_verify_lib.camera_border.manager;
...
public class CameraManager {
    ...
    public synchronized void openDriver(SurfaceHolder holder)
        ...
        if (!initialized) {
            ...
            boolean hasFoundPreferablePreviewSize = false;
            ...
            for (Camera.Size size : previewSizes) {
                ...
                if (size.width - w <= 100) {
                    hasFoundPreferablePreviewSize=true;
                    w = size.width;
                    h = size.height;
                    break;
                }
            ...
            if (hasFoundPreferablePreviewSize) {
            } else {
                if (!ListUtils.isEmpty(previewSizes)){
            ...
            boolean hasFoundPreferablePictureSize = false;
            int pictureW = 800;
            int pictureH = 600;
            for (Camera.Size size : pictureSizes) {
                Log.e("TAG", "pictureSizes width:" + size.width);
                Log.e("TAG", "pictureSizes height:" + size.height);
                if (size.width - pictureW <= 100) {
                    hasFoundPreferablePictureSize=true;
                    pictureW = size.width;
            ...
            if (hasFoundPreferablePictureSize) {
            } else {
                if (!ListUtils.isEmpty(pictureSizes)){
                    Camera.Size first = pictureSizes.get(0);
                    pictureW = first.width;
                    pictureH = first.height;
                }
            }
