...
public class SubsistenceApproveTrendStatisticsFragmentPresenter implements SubsistenceApproveTrendStatisticsFragmentContract.presenter {

    private final SubsistenceApproveTrendStatisticsFragmentContract.View view;
    private final SharedPreferences sharedPreferences;
    private final GCAService GCAService;


    @Inject
    public SubsistenceApproveTrendStatisticsFragmentPresenter(SubsistenceApproveTrendStatisticsFragmentContract.View view,
...
//        RequestBody body = Util.getBodyFromMap(map);
        ...
        GCAService
                .getSubsistenceApproveTrendStatisticsList(ultimateMap)
                .compose(abstractActivity.<GCAHttpResultBaseBean<List<SubsistenceApproveTrendStatisticsBean>>>bindToLifecycle())
//                .compose(abstractActivity.bindUntilEvent(ActivityEvent.DESTROY))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<GCAHttpResultBaseBean<List<SubsistenceApproveTrendStatisticsBean>>>() {
                    @Override
                    public void onStart() {
                        super.onStart();
                    ...
                    @Override
                    public void onNext(GCAHttpResultBaseBean<List<SubsistenceApproveTrendStatisticsBean>> bean) {
//                        Util.decodeBase64Bean(bean);
                        ...
                        if (isEmpty){
                            isSuccessful=true;
                            if (bean!=null){
                                bean.setData(new ArrayList<SubsistenceApproveTrendStatisticsBean>());
                            }
                        }
                        if (isSuccessful) {
