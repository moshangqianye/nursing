...
import com.jqsoft.nursing.adapter.MedicalAssistantFinanceAssuranceStatisticsAdapter;
...
import com.jqsoft.nursing.di.presenter.MedicalAssistantFinanceAssuranceStatisticsFragmentPresenter;
...
public class MedicalAssistantFinanceAssuranceStatisticsFragment extends AbstractFragment implements MedicalAssistantFinanceAssuranceStatisticsFragmentContract.View {
...
//    TextView tvSupportType;
    ...
    String type;//	统计类型	“0”	排名统计    “1”	趋势统计    “2”	资助参保增长率


    String methodName;

    @Inject
    MedicalAssistantFinanceAssuranceStatisticsFragmentPresenter mPresenter;
...
//    String supportType;
    ...
    MedicalAssistantFinanceAssuranceStatisticsAdapter adapter;

    public MedicalAssistantFinanceAssuranceStatisticsFragment() {
    }
    ...
    protected int getLayoutId() {
        return R.layout.fragment_medical_assistant_finance_assurance_statistics_layout;
    }
...
//        tvFailureView.setText(failureString);
        ...
        adapter = new MedicalAssistantFinanceAssuranceStatisticsAdapter(type, new ArrayList<MedicalAssistantFinanceAssuranceBean>());
...
//        });
    ...
    private void initRecyclerViewHeaderVisibilityAndValue(){
        if (Constants.MEDICAL_ASSISTANT_FINANCE_ASSURANCE_RANKING_VALUE.equals(type)){
            ...
            tvTitle.setText("辖区");
            tvFirst.setText("资助人数");
            tvSecond.setText("资助金额(元)");
            tvThird.setText(Constants.EMPTY_STRING);
            tvFourth.setText(Constants.EMPTY_STRING);
        } else if (Constants.MEDICAL_ASSISTANT_FINANCE_ASSURANCE_TREND_VALUE.equals(type)) {
            tvTitle.setVisibility(View.VISIBLE);
            tvFirst.setVisibility(View.VISIBLE);
            tvSecond.setVisibility(View.VISIBLE);
            tvThird.setVisibility(View.GONE);
        ...
        } else if (Constants.MEDICAL_ASSISTANT_FINANCE_ASSURANCE_INCREASE_RATIO_VALUE.equals(type)){
            tvTitle.setVisibility(View.VISIBLE);
            tvFirst.setVisibility(View.VISIBLE);
            tvSecond.setVisibility(View.VISIBLE);
            ...
            tvTitle.setText("辖区");
            tvFirst.setText("资助参保金额同比增长率(%)");
            tvSecond.setText("资助参保人数同比增长率(%)");
            tvThird.setText("资助参保金额环比增长率(%)");
            tvFourth.setText("资助参保人数环比增长率(%)");
...
//        float randomMultiplier = 3* 100000f;
        ...
        List<MedicalAssistantFinanceAssuranceBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList);
        //显示列表
//        adapter.setTitleList(titleList);
        ...
        List<Float> personNumberList = new ArrayList<>();
...
//        } else {
            // create 4 DataSets
            if (Constants.MEDICAL_ASSISTANT_FINANCE_ASSURANCE_RANKING_VALUE.equals(type)){
                set1 = new BarDataSet(yVals1, "资助人数");
                set1.setColor(Constants.CHART_FIRST_COLOR);
                set2 = new BarDataSet(yVals2, "资助金额(元)");
                ...
                set1 = new BarDataSet(yVals1, "金额同比增长率(%)");
                set1.setColor(Constants.CHART_FIRST_COLOR);
                set2 = new BarDataSet(yVals2, "人数同比增长率(%)");
                set2.setColor(Constants.CHART_SECOND_COLOR);
                set3 = new BarDataSet(yVals3, "金额环比增长率(%)");
                set3.setColor(Constants.CHART_THIRD_COLOR);
                set4 = new BarDataSet(yVals4, "人数环比增长率(%)");
...
//        float personNumberMax = Collections.max(personNumberList);
//        float personNumberMaxReal = personNumberMax+Constants.CHART_RANGE_DEVIATION_VALUE_SMALL;
//        float personNumberMin = Collections.min(personNumberList);
//        float personNumberMinReal = personNumberMin-Constants.CHART_RANGE_DEVIATION_VALUE_SMALL;
////        hbcChart.setVisibleYRange(personNumberMinReal, personNumberMaxReal, YAxis.AxisDependency.RIGHT);
//        hbcChart.getAxisLeft().setAxisMinimum(personNumberMinReal);
//        hbcChart.getAxisLeft().setAxisMaximum(personNumberMaxReal);
//
//        float moneyMax = Collections.max(moneyList);
//        float moneyMaxReal = moneyMax+Constants.CHART_RANGE_DEVIATION_VALUE_SMALL;
//        float moneyMin = Collections.min(moneyList);
//        float moneyMinReal = moneyMin-Constants.CHART_RANGE_DEVIATION_VALUE_SMALL;
////        hbcChart.setVisibleYRange(moneyMinReal, moneyMaxReal, YAxis.AxisDependency.RIGHT);
//        hbcChart.getAxisRight().setAxisMinimum(moneyMinReal);
//        hbcChart.getAxisRight().setAxisMaximum(moneyMaxReal);
    ...
    private List<MedicalAssistantFinanceAssuranceBean> getNaturalBeanListFromRawBeanList(
            List<String> titleList,
            List<SubsistenceVarianceRankingSpecificCategoryBean> rawList){
        List<MedicalAssistantFinanceAssuranceBean> resultList = new ArrayList<>();
        try {
            if (ListUtils.isEmpty(rawList) ||
                ...
                if (is5Column()) {
                    ...
                    for (int j = 0; bean0!=null&&bean0.getList()!=null&&j<ListUtils.getSize(bean0.getList()); ++j) {
                        MedicalAssistantFinanceAssuranceBean nb = new MedicalAssistantFinanceAssuranceBean();
...
//        return Constants.EMPTY_STRING;
    ...
    protected void initInject() {
        DaggerApplication.get(getActivity())
                .getAppComponent()
                .addMedicalAssistantFinanceAssuranceStatisticsFragment(new MedicalAssistantFinanceAssuranceStatisticsFragmentModule(this))
                .inject(this);
    }
    ...
    private void loadInfo() {
        Map<String, String> map = getFinanceAssuranceListRequestMap();
        mPresenter.main(map);
    }

    private Map<String, String> getFinanceAssuranceListRequestMap() {
        Map<String, String> map = ParametersFactory.getMedicalAssistantFinanceAssuranceStatisticsListMap(getActivity(),
