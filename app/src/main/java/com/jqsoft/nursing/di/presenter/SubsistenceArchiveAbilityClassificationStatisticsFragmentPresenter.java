...
public class SubsistenceArchiveAbilityClassificationStatisticsFragmentPresenter implements SubsistenceArchiveAbilityClassificationStatisticsFragmentContract.presenter {

    private final SubsistenceArchiveAbilityClassificationStatisticsFragmentContract.View view;
    private final SharedPreferences sharedPreferences;
    private final GCAService GCAService;


    @Inject
    public SubsistenceArchiveAbilityClassificationStatisticsFragmentPresenter(SubsistenceArchiveAbilityClassificationStatisticsFragmentContract.View view,
...
//        RequestBody body = Util.getBodyFromMap(map);
        ...
        GCAService
                .getSubsistenceArchiveAbilityClassificationStatisticsList(ultimateMap)
                .compose(abstractActivity.<GCAHttpResultBaseBean<List<NameValueBean>>>bindToLifecycle())
//                .compose(abstractActivity.bindUntilEvent(ActivityEvent.DESTROY))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<GCAHttpResultBaseBean<List<NameValueBean>>>() {
                    @Override
                    public void onStart() {
                        super.onStart();
                    ...
                    @Override
                    public void onNext(GCAHttpResultBaseBean<List<NameValueBean>> bean) {
//                        Util.decodeBase64Bean(bean);
                        ...
                        if (isEmpty){
                            isSuccessful=true;
                            if (bean!=null){
                                bean.setData(new ArrayList<NameValueBean>());
                            }
                        }
                        if (isSuccessful) {
