...
public class SociaHistoryListPresenter implements SocialHistoryActivityContract.presenter {

    private final SocialHistoryActivityContract.View view;
    private final SharedPreferences sharedPreferences;
    private final GCAService signedResidentClientService;


    @Inject
    public SociaHistoryListPresenter(SocialHistoryActivityContract.View view,
...
//        ultimateMap.put(Constants.REQUEST_KEY, jsonString);
        ...
        signedResidentClientService.getSociaHistoryList(map)
                .compose(abstractActivity.<GCAHttpResultBaseBean<List<SocialListHistoryBean>>>bindToLifecycle())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
//                .subscribe(new Subscriber<GCAHttpResultBaseBean<TreatmentListResultWrapperBean>>() {
                 .subscribe(new BaseSubscriber<GCAHttpResultBaseBean<List<SocialListHistoryBean>>>(context) {
                        @Override
                    public void onStart() {
                        super.onStart();
...
//                        }
                    ...
                    @Override
                    public void onNext(GCAHttpResultBaseBean<List<SocialListHistoryBean>> bean) {
