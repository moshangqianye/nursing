...
public class InstitutionLegalPersonClassificationStatisticsFragmentPresenter implements InstitutionLegalPersonClassificationStatisticsFragmentContract.presenter {

    private final InstitutionLegalPersonClassificationStatisticsFragmentContract.View view;
    private final SharedPreferences sharedPreferences;
    private final GCAService GCAService;


    @Inject
    public InstitutionLegalPersonClassificationStatisticsFragmentPresenter(InstitutionLegalPersonClassificationStatisticsFragmentContract.View view,
...
//        RequestBody body = Util.getBodyFromMap(map);
        ...
        GCAService
                .getInstitutionLegalPersonClassificationStatisticsList(ultimateMap)
                .compose(abstractActivity.<GCAHttpResultBaseBean<List<NameValuePercentBean>>>bindToLifecycle())
//                .compose(abstractActivity.bindUntilEvent(ActivityEvent.DESTROY))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<GCAHttpResultBaseBean<List<NameValuePercentBean>>>() {
                    @Override
                    public void onStart() {
                        super.onStart();
                    ...
                    @Override
                    public void onNext(GCAHttpResultBaseBean<List<NameValuePercentBean>> bean) {
//                        Util.decodeBase64Bean(bean);
                        ...
                        if (isEmpty){
                            isSuccessful=true;
                            if (bean!=null){
                                bean.setData(new ArrayList<NameValuePercentBean>());
                            }
                        }
                        if (isSuccessful) {
