...
import com.jqsoft.nursing.adapter.SubsistenceAccountTrendStatisticsAdapter;
...
import com.jqsoft.nursing.di.presenter.SubsistenceAccountTrendStatisticsFragmentPresenter;
...
public class SubsistenceAccountTrendStatisticsFragment extends AbstractFragment implements SubsistenceAccountTrendStatisticsFragmentContract.View {
    ...
    LineChart lcChart;
    @BindView(R.id.lay_subsistence_account_trend_load_failure)
    View failureView;
...
//    String rescueType;//(城乡低保:99	全部  01	农村低保    02	城市低保)   (特困人员供养：03） （低收入家庭：04）
    ...
    @Inject
    SubsistenceAccountTrendStatisticsFragmentPresenter mPresenter;
    ...
    SubsistenceAccountTrendStatisticsAdapter adapter;

    public SubsistenceAccountTrendStatisticsFragment() {
    }
    ...
    protected int getLayoutId() {
        return R.layout.fragment_subsistence_account_trend_statistics_layout;
    }
...
//        tvFailureView.setText(failureString);
        ...
        adapter = new SubsistenceAccountTrendStatisticsAdapter(new ArrayList<SubsistenceAccountTrendNaturalBean>());
...
//        lcChart.animateY(Constants.CHART_ANIMATION_DURATION);
    ...
    private void simulateListData(List<SubsistenceAccountTrendNaturalBean> naturalList){
        for (int i =  0; i < 6; ++i){
            SubsistenceAccountTrendNaturalBean bean  = new SubsistenceAccountTrendNaturalBean("month"+i, ""+i);
...
//        float randomMultiplier = 3* 100000f;
        ...
        List<SubsistenceAccountTrendNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList);
        //显示列表
...
//        } else {
            // create 4 DataSets
            set1 = new LineDataSet(yVals1, "金额");
...
//    }
    ...
    private List<SubsistenceAccountTrendNaturalBean> getNaturalBeanListFromRawBeanList(
            List<String> titleList,
            List<SubsistenceVarianceTrendSpecificCategoryBean> rawList){
        List<SubsistenceAccountTrendNaturalBean> resultList = new ArrayList<>();
        try {
...
//                    SubsistenceVarianceTrendSpecificCategoryBean bean2 = rawList.get(2);
                    for (int j = 0; bean0!=null&&bean0.getList()!=null&&j<ListUtils.getSize(bean0.getList()); ++j) {
                        SubsistenceAccountTrendNaturalBean nb = new SubsistenceAccountTrendNaturalBean();
                        nb.setMonthName(titleList.get(j));
                        nb.setMoney(Util.getNonscientificNumberStringFromString(bean0.getList().get(j)));
...
//        return Constants.EMPTY_STRING;
    ...
    protected void initInject() {
        DaggerApplication.get(getActivity())
                .getAppComponent()
                .addSubsistenceAccountTrendStatisticsFragment(new SubsistenceAccountTrendStatisticsFragmentModule(this))
                .inject(this);
    }
    ...
    private Map<String, String> getTrendListRequestMap() {
        Map<String, String> map = ParametersFactory.getSubsistenceAccountTrendStatisticsListMap(getActivity(), subsistenceType,
