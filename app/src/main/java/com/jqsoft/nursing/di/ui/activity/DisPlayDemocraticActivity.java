...
import com.jqsoft.nursing.di.presenter.DisPlayDemoCraticPresenter;
 ...
 */
//评议回显页面
public class DisPlayDemocraticActivity extends AbstractActivity implements DisPlayDemoCraticContract.View {
    ...
    private String gId;
    @Inject
    DisPlayDemoCraticPresenter mPresenter;

    @Override

    protected int getLayoutId() {
        return R.layout.activity_addservery;
    }
    ...
    protected void initView() {
        ...
        final FullyGridLayoutManager manager = new FullyGridLayoutManager(DisPlayDemocraticActivity.this, 4, GridLayoutManager.VERTICAL, false);
        recyclerView.setLayoutManager(manager);
        adapter = new GridImageHouseHoldAdapter(DisPlayDemocraticActivity.this, onAddPicClickListener);
        ...
        recyclerView.setAdapter(adapter);
         ...
         adapter.setOnItemClickListener(new GridImageHouseHoldAdapter.OnItemClickListener() {
             ...
             public void onItemClick(int position, View v) {
                 PictureConfig.getInstance().externalPicturePreview(DisPlayDemocraticActivity.this, position, selectMedia);
             }
         });
    }
    ...
    protected void initInject() {
        DaggerApplication.get(this)
                .getAppComponent()
                .addDisplayDemoCratic(new DisPlayDemocraticModule(this))
                .inject(this);
    }

    @Override
    public void onLoadListSuccess(GCAHttpResultBaseBean<DemoCraticBaseBean> bean) {
          bean.getData().getFileList();
    ...
    @Override
    public void onLoadMoreListSuccess(GCAHttpResultBaseBean<DemoCraticBaseBean> bean) {
        ...
        public void onAddPicClick(int type, int position) {
            switch (type) {
                case 0:
                    ...
                    if (theme) {
                        // 设置主题样式
                        themeStyle = ContextCompat.getColor(DisPlayDemocraticActivity.this, R.color.blue);
                    } else {
                        themeStyle = ContextCompat.getColor(DisPlayDemocraticActivity.this, R.color.bar_grey);
                    }
                    ...
                    if (isCheckNumMode) {
                        // QQ 风格模式下 这里自己搭配颜色
                        previewColor = ContextCompat.getColor(DisPlayDemocraticActivity.this, R.color.blue);
                        completeColor = ContextCompat.getColor(DisPlayDemocraticActivity.this, R.color.blue);
                    } else {
                        previewColor = ContextCompat.getColor(DisPlayDemocraticActivity.this, R.color.tab_color_true);
                        completeColor = ContextCompat.getColor(DisPlayDemocraticActivity.this, R.color.tab_color_true);
                    }
...
//                            .setImmersive(true)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(DisPlayDemocraticActivity.this);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(DisPlayDemocraticActivity.this, resultCallback);
                    }
                    break;
                case 1:
                    // 删除图片
