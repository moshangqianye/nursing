...
public class FamilyEconomyCheckProjectCheckStatisticsFragmentPresenter implements FamilyEconomyCheckProjectCheckStatisticsFragmentContract.presenter {

    private final FamilyEconomyCheckProjectCheckStatisticsFragmentContract.View view;
    private final SharedPreferences sharedPreferences;
    private final GCAService GCAService;


    @Inject
    public FamilyEconomyCheckProjectCheckStatisticsFragmentPresenter(FamilyEconomyCheckProjectCheckStatisticsFragmentContract.View view,
...
//        RequestBody body = Util.getBodyFromMap(map);
        ...
        GCAService
                .getFamilyEconomyCheckProjectCheckStatisticsList(ultimateMap)
                .compose(abstractActivity.<GCAHttpResultBaseBean<List<FamilyEconomyCheckProjectCheckBean>>>bindToLifecycle())
//                .compose(abstractActivity.bindUntilEvent(ActivityEvent.DESTROY))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<GCAHttpResultBaseBean<List<FamilyEconomyCheckProjectCheckBean>>>() {
                    @Override
                    public void onStart() {
                        super.onStart();
                    ...
                    @Override
                    public void onNext(GCAHttpResultBaseBean<List<FamilyEconomyCheckProjectCheckBean>> bean) {
//                        Util.decodeBase64Bean(bean);
                        ...
                        if (isEmpty){
                            isSuccessful=true;
                            if (bean!=null){
                                bean.setData(new ArrayList<FamilyEconomyCheckProjectCheckBean>());
                            }
                        }
                        if (isSuccessful) {
