...
import com.jqsoft.nursing.adapter.SubsistenceArchiveRankingStatisticsAdapter;
...
import com.jqsoft.nursing.di.presenter.SubsistenceArchiveRankingStatisticsFragmentPresenter;
...
/**
 * 柱状图
 * 低保档案情况/特困人员档案情况/低收入家庭档案情况排名统计
 * Created by Administrator on 2018-01-02.
 */

public class SubsistenceArchiveRankingStatisticsFragment extends AbstractFragment implements SubsistenceArchiveRankingStatisticsFragmentContract.View {
    ...
    @BindView(R.id.lay_subsistence_archive_ranking_load_failure)
    View failureView;
...
//    String rescueType;//(城乡低保:99	全部  01	农村低保    02	城市低保)   (特困人员供养：03） （低收入家庭：04）
    ...
    @Inject
    SubsistenceArchiveRankingStatisticsFragmentPresenter mPresenter;
    ...
    SubsistenceArchiveRankingStatisticsAdapter adapter;

    public SubsistenceArchiveRankingStatisticsFragment() {
    }
    ...
    protected int getLayoutId() {
        return R.layout.fragment_subsistence_archive_ranking_statistics_layout;
    }
...
//        tvFailureView.setText(failureString);
        ...
        adapter = new SubsistenceArchiveRankingStatisticsAdapter(new ArrayList<SubsistenceArchiveRankingNaturalBean>());
...
//        List<SubsistenceArchiveRankingNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList);
//        //显示列表
...
//        } else {
            // create 4 DataSets
            set1 = new BarDataSet(yVals1, "户数");
            set1.setColor(Constants.CHART_FIRST_COLOR);
            set2 = new BarDataSet(yVals2, "人数");
            set2.setColor(Constants.CHART_SECOND_COLOR);
//            set3 = new BarDataSet(yVals3, "注销");
//            set3.setColor(Constants.CHART_THIRD_COLOR);
...
//        hbcChart.setVisibleXRangeMaximum(Constants.NUMBER_PER_DISTRICT * yVals1.size() * barWidth);
    ...
    private List<SubsistenceArchiveRankingNaturalBean> getNaturalBeanListFromRawBeanList(
            List<String> titleList,
            List<SubsistenceVarianceRankingSpecificCategoryBean> rawList){
        List<SubsistenceArchiveRankingNaturalBean> resultList = new ArrayList<>();
        try {
...
//                    SubsistenceVarianceRankingSpecificCategoryBean bean2 = rawList.get(2);
                    for (int j = 0; bean0!=null&&bean0.getList()!=null&&j<ListUtils.getSize(bean0.getList()); ++j) {
                        SubsistenceArchiveRankingNaturalBean nb = new SubsistenceArchiveRankingNaturalBean();
                        nb.setDistrictName(titleList.get(j));
                        nb.setHouseholdNumber(Util.getForcedIntStringFromDoubleString(bean0.getList().get(j)));
                        nb.setPersonNumber(Util.getForcedIntStringFromDoubleString(bean1.getList().get(j)));
...
//        return Constants.EMPTY_STRING;
    ...
    protected void initInject() {
        DaggerApplication.get(getActivity())
                .getAppComponent()
                .addSubsistenceArchiveRankingStatisticsFragment(new SubsistenceArchiveRankingStatisticsFragmentModule(this))
                .inject(this);
    }
    ...
    private Map<String, String> getRankingListRequestMap() {
        Map<String, String> map = ParametersFactory.getSubsistenceArchiveRankingStatisticsListMap(getActivity(), subsistenceType,
