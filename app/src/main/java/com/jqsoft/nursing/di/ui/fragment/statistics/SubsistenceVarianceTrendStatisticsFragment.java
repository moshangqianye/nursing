...
import com.jqsoft.nursing.adapter.SubsistenceVarianceTrendStatisticsAdapter;
...
import com.jqsoft.nursing.di.presenter.SubsistenceVarianceTrendStatisticsFragmentPresenter;
...
/**
 * 折线图
 * 低保变化情况/特困人员变化情况/低收入家庭情况趋势分析
 * Created by Administrator on 2018-01-02.
 */

public class SubsistenceVarianceTrendStatisticsFragment extends AbstractFragment implements SubsistenceVarianceTrendStatisticsFragmentContract.View {
    ...
    LineChart lcChart;
    @BindView(R.id.lay_subsistence_variance_trend_load_failure)
    View failureView;
...
//    String rescueType;//(城乡低保:99	全部  01	农村低保    02	城市低保)   (特困人员供养：03） （低收入家庭：04）
    ...
    @Inject
    SubsistenceVarianceTrendStatisticsFragmentPresenter mPresenter;
    ...
    SubsistenceVarianceTrendStatisticsAdapter adapter;

    public SubsistenceVarianceTrendStatisticsFragment() {
    }
    ...
    protected int getLayoutId() {
        return R.layout.fragment_subsistence_variance_trend_statistics_layout;
    }
...
//        tvFailureView.setText(failureString);
        ...
        adapter = new SubsistenceVarianceTrendStatisticsAdapter(new ArrayList<SubsistenceVarianceTrendNaturalBean>());
...
//        lcChart.animateY(Constants.CHART_ANIMATION_DURATION);
    ...
    private void simulateListData(List<SubsistenceVarianceTrendNaturalBean> naturalList){
        for (int i =  0; i < 6; ++i){
            SubsistenceVarianceTrendNaturalBean bean  = new SubsistenceVarianceTrendNaturalBean("month"+i, ""+i, ""+i, ""+i);
...
//        float randomMultiplier = 3* 100000f;
        ...
        List<SubsistenceVarianceTrendNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList);
        //显示列表
...
//        } else {
            // create 4 DataSets
            set1 = new LineDataSet(yVals1, "新增");
            ...
            set2 = new LineDataSet(yVals2, "复查");
...
//    }
    ...
    private List<SubsistenceVarianceTrendNaturalBean> getNaturalBeanListFromRawBeanList(
            List<String> titleList,
            List<SubsistenceVarianceTrendSpecificCategoryBean> rawList){
        List<SubsistenceVarianceTrendNaturalBean> resultList = new ArrayList<>();
        try {
...
//                for (int i = 0; i < ListUtils.getSize(rawList); ++i){
                    ...
                    for (int j = 0; bean0!=null&&bean0.getList()!=null&&j<ListUtils.getSize(bean0.getList()); ++j) {
                        SubsistenceVarianceTrendNaturalBean nb = new SubsistenceVarianceTrendNaturalBean();
...
//        return Constants.EMPTY_STRING;
    ...
    protected void initInject() {
        DaggerApplication.get(getActivity())
                .getAppComponent()
                .addSubsistenceVarianceTrendStatisticsFragment(new SubsistenceVarianceTrendStatisticsFragmentModule(this))
                .inject(this);
    }
    ...
    private Map<String, String> getTrendListRequestMap() {
        Map<String, String> map = ParametersFactory.getSubsistenceVarianceTrendStatisticsListMap(getActivity(), subsistenceType,
                resultType, supportType, getYear(), getIsAreaOwner(), getAreaCodeString(), methodName);
        return map;
    }
