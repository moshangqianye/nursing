...
import com.jqsoft.nursing.adapter.SubsistenceAccountRankingStatisticsAdapter;
...
import com.jqsoft.nursing.di.presenter.SubsistenceAccountRankingStatisticsFragmentPresenter;
...
/**
 * 柱状图
 * 低保档案情况/特困人员档案情况 资金台账 排名统计
 * Created by Administrator on 2018-01-02.
 */

public class SubsistenceAccountRankingStatisticsFragment extends AbstractFragment implements SubsistenceAccountRankingStatisticsFragmentContract.View {
    ...
    @BindView(R.id.lay_subsistence_account_ranking_load_failure)
    View failureView;
...
//    String rescueType;//(城乡低保:99	全部  01	农村低保    02	城市低保)   (特困人员供养：03） （低收入家庭：04）
    ...
    @Inject
    SubsistenceAccountRankingStatisticsFragmentPresenter mPresenter;
    ...
    SubsistenceAccountRankingStatisticsAdapter adapter;

    public SubsistenceAccountRankingStatisticsFragment() {
    }
    ...
    protected int getLayoutId() {
        return R.layout.fragment_subsistence_account_ranking_statistics_layout;
    }
...
//        tvFailureView.setText(failureString);
        ...
        adapter = new SubsistenceAccountRankingStatisticsAdapter(new ArrayList<SubsistenceAccountRankingNaturalBean>());
...
//        List<SubsistenceAccountRankingNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList);
//        //显示列表
...
//        hbcChart.setVisibleXRangeMinimum(Constants.BAR_NUMBER_PER_SCREEN_FOR_ONE_COLUMN*barWidth);
    ...
    private List<SubsistenceAccountRankingNaturalBean> getNaturalBeanListFromRawBeanList(
            List<String> titleList,
            List<SubsistenceVarianceRankingSpecificCategoryBean> rawList){
        List<SubsistenceAccountRankingNaturalBean> resultList = new ArrayList<>();
        try {
...
//                    SubsistenceVarianceRankingSpecificCategoryBean bean2 = rawList.get(2);
                    for (int j = 0; bean0!=null&&bean0.getList()!=null&&j<ListUtils.getSize(bean0.getList()); ++j) {
                        SubsistenceAccountRankingNaturalBean nb = new SubsistenceAccountRankingNaturalBean();
...
//        return Constants.EMPTY_STRING;
    ...
    protected void initInject() {
        DaggerApplication.get(getActivity())
                .getAppComponent()
                .addSubsistenceAccountRankingStatisticsFragment(new SubsistenceAccountRankingStatisticsFragmentModule(this))
                .inject(this);
    }
    ...
    private Map<String, String> getRankingListRequestMap() {
        Map<String, String> map = ParametersFactory.getSubsistenceAccountRankingStatisticsListMap(getActivity(), subsistenceType,
