...
public class RoundRoomFragmentPresenter implements SocialAssistanceObjectActivityContract.presenter {
    ...
    @Inject
    public RoundRoomFragmentPresenter(SocialAssistanceObjectActivityContract.View view,
...
//        ultimateMap.put(Constants.REQUEST_KEY, jsonString);
        ...
        signedResidentClientService.getQrcodeInfo(map)
                .compose(abstractFragment.<HttpResultNurseBaseBean<String>>bindToLifecycle())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
//                .subscribe(new Subscriber<GCAHttpResultBaseBean<TreatmentListResultWrapperBean>>() {
...
//                        }
    ...
    public void getmacInfo(Map<String, String> map, final boolean isLoadMore) {
        Fragment fragment = (Fragment)view;
        AbstractFragment abstractFragment = (AbstractFragment) fragment;
        final Context context = (Context)fragment.getActivity();
        if (!Util.isNetworkConnected(context)){
            view.onLoadmacFailure(Constants.HINT_LOADING_DATA_FAILURE, isLoadMore);
            return;
        }
//        Map<String, String> ultimateMap = new HashMap<>();
//        String jsonString = JSON.toJSONString(map);
//        ultimateMap.put(Constants.REQUEST_KEY, jsonString);
        ...
        signedResidentClientService.getmacInfo(map)
                .compose(abstractFragment.<HttpResultNurseBaseBean<String>>bindToLifecycle())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
//                .subscribe(new Subscriber<GCAHttpResultBaseBean<TreatmentListResultWrapperBean>>() {
...
//                        });
                    ...
                    public void onError(Throwable e) {
                        super.onError(e);
                        view.onLoadmacFailure(e.getMessage(), isLoadMore);
//                        Activity activity = (Activity) view;
//                        Util.hideGifProgressDialog(activity);
...
//                        }
                            ...
                            String msg = Util.getNursingMessageFromHttpResponse(bean);
                            view.onLoadmacFailure(msg,false);
                        }
