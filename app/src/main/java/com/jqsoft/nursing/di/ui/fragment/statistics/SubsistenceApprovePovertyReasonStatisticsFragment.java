...
import com.jqsoft.nursing.adapter.SubsistenceApprovePovertyReasonStatisticsAdapter;
...
import com.jqsoft.nursing.di.presenter.SubsistenceApprovePovertyReasonStatisticsFragmentPresenter;
...
/**
 * 饼状图
 * 低保审批情况/特困人员供养/低收入家庭 致贫原因/退保注销原因 分类统计
 * Created by Administrator on 2018-01-02.
 */

public class SubsistenceApprovePovertyReasonStatisticsFragment extends AbstractFragment implements SubsistenceApprovePovertyReasonStatisticsFragmentContract.View {
...
//    TextView tvResultType;
    ...
    @Inject
    SubsistenceApprovePovertyReasonStatisticsFragmentPresenter mPresenter;
...
//    String resultType;
    ...
    SubsistenceApprovePovertyReasonStatisticsAdapter adapter;

    public SubsistenceApprovePovertyReasonStatisticsFragment() {
    }
    ...
    private void populateData() {
        ...
        statisticsType=getDeliveredString(Constants.STATISTICS_POVERTY_REASON_KEY);
    }
...
//        tvFailureView.setText(failureString);
        ...
        adapter = new SubsistenceApprovePovertyReasonStatisticsAdapter(new ArrayList<SubsistenceApprovePovertyReasonNaturalBean>());
...
//        pcChart.setDrawEntryLabels(false);
    ...
    private void setData(List<SubsistenceApprovePovertyReasonBean> beanList) {
        List<SubsistenceApprovePovertyReasonNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(beanList);
        //显示列表
//        adapter.setTitleList(titleList);
        ...
        boolean b = Util.isItemPercentVerySmallCountLessThanFixedNumberFromSubsistenceApprovePovertyReasonNaturalBeanList(naturalList);

        // NOTE: The order of the entries when being added to the entries array determines their position around the center of
        // the chart.
        if (b){
            for (int i = 0; i < beanList.size() ; i++) {
                SubsistenceApprovePovertyReasonNaturalBean bean = naturalList.get(i);
//            entries.add(new PieEntry((float) Util.getFloatFromPercentString(String.valueOf(bean.getValue())),
//                    Constants.EMPTY_STRING));
                float value = (float) Util.getFloatFromPercentString(String.valueOf(bean.getPercent()));
...
//                    bean.getName()+Constants.SPACE_STRING+bean.getValue()));
            ...
            for (int i = 0; i < beanList.size(); i++) {
                SubsistenceApprovePovertyReasonNaturalBean bean = naturalList.get(i);
                float value = (float) Util.getFloatFromPercentString(String.valueOf(bean.getPercent()));
                boolean isSmall = Util.isItemPercentVerySmallInSubsistenceApprovePovertyReasonNaturalBeanList(value, naturalList);
                if (!isSmall) {
                    entries.add(new PieEntry(value,
                            bean.getTitle()));
                }
...
//        for (int i = 0; i < beanList.size() ; i++) {
//            SubsistenceApprovePovertyReasonNaturalBean bean = naturalList.get(i);
//            entries.add(new PieEntry((float) Util.getFloatFromPercentString(String.valueOf(bean.getPercent())),
//                    bean.getTitle()));
////            entries.add(new PieEntry((float) Util.getFloatFromPercentString(String.valueOf(bean.getPercent())),
////                    bean.getTitle()+Constants.SPACE_STRING+bean.getHouseholdNumber()));
//        }
...
//        dataSet.setValueLinePart2Length(2f);
        ...
        List<Float> fl = Util.getFloatListFromSubsistenceApprovePovertyReasonNaturalBeanList(naturalList);
...
//            set.setDrawValues(true);
    ...
    private List<SubsistenceApprovePovertyReasonNaturalBean> getNaturalBeanListFromRawBeanList(
            List<SubsistenceApprovePovertyReasonBean> rawList){
        List<SubsistenceApprovePovertyReasonNaturalBean> resultList = new ArrayList<>();
        try {
            if (ListUtils.isEmpty(rawList)){
                return resultList;
                    ...
                    for (int j = 0; j < rawList.size(); ++j) {
                        ...
                        SubsistenceApprovePovertyReasonNaturalBean nb = new SubsistenceApprovePovertyReasonNaturalBean();
                        nb.setTitle(b.getName());
                        nb.setHouseholdNumber(Util.getForcedIntStringFromDoubleString(b.getValue()));
...
//        return Constants.EMPTY_STRING;
    ...
    protected void initInject() {
        DaggerApplication.get(getActivity())
                .getAppComponent()
                .addPovertyReasonStatisticsFragment(new SubsistenceApprovePovertyReasonStatisticsFragmentModule(this))
                .inject(this);
    }
    ...
    private void loadInfo() {
        Map<String, String> map = getPovertyReasonListRequestMap();
        mPresenter.main(map);
    }

    private Map<String, String> getPovertyReasonListRequestMap() {
        Map<String, String> map = ParametersFactory.getSubsistenceApprovePovertyReasonStatisticsListMap(getActivity(), subsistenceType,
                supportType, statisticsType, getMonth(), getIsAreaOwner(), getAreaCodeString(), methodName);
        return map;
    }

    private List<SubsistenceApprovePovertyReasonBean> getStatisticsBean(GCAHttpResultBaseBean<List<SubsistenceApprovePovertyReasonBean>> bean) {
        if (bean != null) {
            List<SubsistenceApprovePovertyReasonBean> list = bean.getData();
            return list;
        } else {
            return null;
        }
    ...
    @Override
    public void onLoadListSuccess(GCAHttpResultBaseBean<List<SubsistenceApprovePovertyReasonBean>> bean) {
        List<SubsistenceApprovePovertyReasonBean> b = getStatisticsBean(bean);
...
//        simulate();
    ...
    private void simulate() {
        ...
        List<SubsistenceApprovePovertyReasonBean> list = new ArrayList<>();
        for (int i = 0; i < titleArray.length; ++i){
            SubsistenceApprovePovertyReasonBean b = new SubsistenceApprovePovertyReasonBean(String.valueOf(100+i*50),
                    33, titleList.get(i));
            list.add(b);
        }
