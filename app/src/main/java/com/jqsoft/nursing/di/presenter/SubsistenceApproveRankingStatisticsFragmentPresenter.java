...
public class SubsistenceApproveRankingStatisticsFragmentPresenter implements SubsistenceApproveRankingStatisticsFragmentContract.presenter {

    private final SubsistenceApproveRankingStatisticsFragmentContract.View view;
    private final SharedPreferences sharedPreferences;
    private final GCAService GCAService;


    @Inject
    public SubsistenceApproveRankingStatisticsFragmentPresenter(SubsistenceApproveRankingStatisticsFragmentContract.View view,
...
//        RequestBody body = Util.getBodyFromMap(map);
        ...
        GCAService
                .getSubsistenceApproveRankingStatisticsList(ultimateMap)
                .compose(abstractActivity.<GCAHttpResultBaseBean<List<SubsistenceApproveRankingStatisticsBean>>>bindToLifecycle())
//                .compose(abstractActivity.bindUntilEvent(ActivityEvent.DESTROY))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<GCAHttpResultBaseBean<List<SubsistenceApproveRankingStatisticsBean>>>() {
                    @Override
                    public void onStart() {
                        super.onStart();
                    ...
                    @Override
                    public void onNext(GCAHttpResultBaseBean<List<SubsistenceApproveRankingStatisticsBean>> bean) {
//                        Util.decodeBase64Bean(bean);
                        ...
                        if (isEmpty){
                            isSuccessful=true;
                            if (bean!=null){
                                bean.setData(new ArrayList<SubsistenceApproveRankingStatisticsBean>());
                            }
                        }
                        if (isSuccessful) {
