...
import com.jqsoft.nursing.adapter.SubsistenceStandardRankingStatisticsAdapter;
...
import com.jqsoft.nursing.di.presenter.SubsistenceStandardRankingStatisticsFragmentPresenter;
...
public class SubsistenceStandardRankingStatisticsFragment extends AbstractFragment implements SubsistenceStandardRankingStatisticsFragmentContract.View {
    ...
    @BindView(R.id.lay_subsistence_standard_ranking_load_failure)
    View failureView;
...
//    String rescueType;//(城乡低保:99	全部  01	农村低保    02	城市低保)   (特困人员供养：03） （低收入家庭：04）
    ...
    @Inject
    SubsistenceStandardRankingStatisticsFragmentPresenter mPresenter;
    ...
    SubsistenceStandardRankingStatisticsAdapter adapter;

    public SubsistenceStandardRankingStatisticsFragment() {
    }
    ...
    protected int getLayoutId() {
        return R.layout.fragment_subsistence_standard_ranking_statistics_layout;
    }
...
//        tvFailureView.setText(failureString);
        ...
        adapter = new SubsistenceStandardRankingStatisticsAdapter(new ArrayList<SubsistenceStandardRankingNaturalBean>());
...
//        float randomMultiplier = 3* 100000f;
        ...
        List<SubsistenceStandardRankingNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList);
        //显示列表
...
//        } else {
            // create 4 DataSets
            set1 = new BarDataSet(yVals1, "标准(元/月/人)");
...
//
//    }
    private List<SubsistenceStandardRankingNaturalBean> getNaturalBeanListFromRawBeanList(
            List<String> titleList,
            List<SubsistenceVarianceRankingSpecificCategoryBean> rawList){
        List<SubsistenceStandardRankingNaturalBean> resultList = new ArrayList<>();
        try {
...
//                    SubsistenceVarianceRankingSpecificCategoryBean bean2 = rawList.get(2);
                    for (int j = 0; bean0!=null&&bean0.getList()!=null&&j<ListUtils.getSize(bean0.getList()); ++j) {
                        SubsistenceStandardRankingNaturalBean nb = new SubsistenceStandardRankingNaturalBean();
                        nb.setDistrictName(titleList.get(j));
                        nb.setMoneyPerMonthPerson(Util.getNonscientificNumberStringFromString(bean0.getList().get(j)));
...
//        return Constants.EMPTY_STRING;
    ...
    protected void initInject() {
        DaggerApplication.get(getActivity())
                .getAppComponent()
                .addSubsistenceStandardRankingStatisticsFragment(new SubsistenceStandardRankingStatisticsFragmentModule(this))
                .inject(this);
    }
    ...
    private Map<String, String> getRankingListRequestMap() {
        Map<String, String> map = ParametersFactory.getSubsistenceStandardRankingStatisticsListMap(getActivity(), subsistenceType,
