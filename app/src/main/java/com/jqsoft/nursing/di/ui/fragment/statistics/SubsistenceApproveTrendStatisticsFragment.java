...
import com.jqsoft.nursing.adapter.SubsistenceApproveTrendStatisticsAdapter;
...
import com.jqsoft.nursing.bean.grassroots_civil_administration.SubsistenceApproveTrendSpecificCategoryBean;
...
import com.jqsoft.nursing.di.presenter.SubsistenceApproveTrendStatisticsFragmentPresenter;
...
/**
 * 低保审批情况/特困人员审批情况/低收入家庭审批情况趋势分析
 * Created by Administrator on 2018-01-02.
 */

public class SubsistenceApproveTrendStatisticsFragment extends AbstractFragment implements SubsistenceApproveTrendStatisticsFragmentContract.View {
    ...
    LineChart lcChart;
    @BindView(R.id.lay_subsistence_approve_trend_load_failure)
    View failureView;
    ...
    @Inject
    SubsistenceApproveTrendStatisticsFragmentPresenter mPresenter;
    ...
    SubsistenceApproveTrendStatisticsAdapter adapter;

    public SubsistenceApproveTrendStatisticsFragment() {
    }
    ...
    protected int getLayoutId() {
        return R.layout.fragment_subsistence_approve_trend_statistics_layout;
    }
...
//        tvFailureView.setText(failureString);
        ...
        adapter = new SubsistenceApproveTrendStatisticsAdapter(new ArrayList<SubsistenceApproveTrendNaturalBean>());
...
//        lcChart.animateY(Constants.CHART_ANIMATION_DURATION);
    ...
    private void setData(SubsistenceApproveTrendStatisticsBean bean) {
...
//        float randomMultiplier = 3* 100000f;
        ...
        List<SubsistenceApproveTrendSpecificCategoryBean> categoryList = bean.getList();
        List<SubsistenceApproveTrendNaturalBean> naturalList = getNaturalBeanListFromRawBeanList(titleList, categoryList);
        //显示列表
//        adapter.setTitleList(titleList);
        ...
        for (int j = 0; j < categoryList.size(); ++j) {
            SubsistenceApproveTrendSpecificCategoryBean cb = categoryList.get(j);
...
//        } else {
            // create 4 DataSets
            set1 = new LineDataSet(yVals1, "通过率");
...
//        lcChart.setVisibleXRangeMaximum(6*barWidth);
        lcChart.invalidate();
...
//    }
//
    private List<SubsistenceApproveTrendNaturalBean> getNaturalBeanListFromRawBeanList(
            List<String> titleList,
            List<SubsistenceApproveTrendSpecificCategoryBean> rawList){
        List<SubsistenceApproveTrendNaturalBean> resultList = new ArrayList<>();
        try {
...
//                for (int i = 0; i < ListUtils.getSize(rawList); ++i){
                    SubsistenceApproveTrendSpecificCategoryBean bean0 = rawList.get(0);
//                    SubsistenceApproveTrendSpecificCategoryBean bean1 = rawList.get(1);
//                    SubsistenceApproveTrendSpecificCategoryBean bean2 = rawList.get(2);
                    for (int j = 0; bean0!=null&&bean0.getList()!=null&&j<ListUtils.getSize(bean0.getList()); ++j) {
                        SubsistenceApproveTrendNaturalBean nb = new SubsistenceApproveTrendNaturalBean();
...
//        return Constants.EMPTY_STRING;
    ...
    protected void initInject() {
        DaggerApplication.get(getActivity())
                .getAppComponent()
                .addSubsistenceApproveTrendStatisticsFragment(new SubsistenceApproveTrendStatisticsFragmentModule(this))
                .inject(this);
    }
    ...
    private void loadInfo() {
        Map<String, String> map = getTrendListRequestMap();
        mPresenter.main(map);
    }

    private Map<String, String> getTrendListRequestMap() {
        Map<String, String> map = ParametersFactory.getSubsistenceApproveTrendStatisticsListMap(getActivity(), subsistenceType,
                supportType, getYear(), getIsAreaOwner(), getAreaCodeString(), methodName);
        return map;
    }

    private SubsistenceApproveTrendStatisticsBean getStatisticsBean(GCAHttpResultBaseBean<List<SubsistenceApproveTrendStatisticsBean>> bean) {
        if (bean != null) {
            List<SubsistenceApproveTrendStatisticsBean> list = bean.getData();
            if (!ListUtils.isEmpty(list)) {
                return list.get(0);
            } else {
                return null;
    ...
    @Override
    public void onLoadListSuccess(GCAHttpResultBaseBean<List<SubsistenceApproveTrendStatisticsBean>> bean) {
        SubsistenceApproveTrendStatisticsBean b = getStatisticsBean(bean);
...
//        List<SubsistenceApproveTrendSpecificCategoryBean> dataList = new ArrayList<>();
////        for (int i = 0; i < 3; ++i) {
//            List<Float> intList = new ArrayList<>();
//            for (int j = 0; j < titleArray.length; ++j) {
//                intList.add(60f+j*30);
//            }
//            SubsistenceApproveTrendSpecificCategoryBean b = new SubsistenceApproveTrendSpecificCategoryBean("通过率", intList);
//            dataList.add(b);
////        }
//        SubsistenceApproveTrendStatisticsBean bean = new SubsistenceApproveTrendStatisticsBean(titleList, dataList);
//        setData(bean);
    }
}
