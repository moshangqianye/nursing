...
public class SignClientServiceAssessActivityPresenter implements SignClientServiceAssessActivityContract.presenter {

    private final SignClientServiceAssessActivityContract.View view;
    private final SharedPreferences sharedPreferences;
    private final GCAService GCAService;


    @Inject
    public SignClientServiceAssessActivityPresenter(SignClientServiceAssessActivityContract.View view,
    ...
    public void main(Map<String, String> map, final boolean isLoadMore) {
        ...
        GCAService.getPersonEvaluationItems(body)
                .compose(abstractActivity.<HttpResultBaseBean<List<SignClientServiceAssessResultBean>>>bindToLifecycle())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
//                .subscribe(new Subscriber<HttpResultBaseBean<TreatmentListResultWrapperBean>>() {
                .subscribe(new BaseSubscriber<HttpResultBaseBean<List<SignClientServiceAssessResultBean>>>(context) {
                    @Override
                    public void onStart() {
                        super.onStart();
...
//                        }
                    ...
                    @Override
                    public void onNext(HttpResultBaseBean<List<SignClientServiceAssessResultBean>> bean) {
    ...
    public void readItem(Map<String, String> map) {
        final Context context = (Context) view;
        AbstractActivity abstractActivity = (AbstractActivity) context;
        if (!Util.isNetworkConnected(context)) {
            view.onReadSignServiceAssessItemFailure(Constants.HINT_LOADING_DATA_FAILURE);
            return;
        }
        RequestBody body = Util.getBodyFromMap(map);

        GCAService.readSignServiceAssessItem(body)
...
//                        });
                    ...
                    public void onError(Throwable e) {
                        super.onError(e);
                        view.onReadSignServiceAssessItemFailure(e.getMessage());
//                        Activity activity = (Activity) view;
//                        Util.hideGifProgressDialog(activity);
...
//                        }
                    ...
                    public void onNext(HttpResultBaseBean<HttpResultEmptyBean> bean) {
                        ...
                        boolean isResponseTokenTimeout = Util.isResponseTokenTimeout(bean);
                        if (isSuccessful) {
                            view.onReadSignServiceAssessItemSuccess(bean);
                        } else if (isResponseTokenTimeout) {
                            Util.gotoLoginActivity(context, true, true);
                        } else {
                            String msg = Util.getMessageFromHttpResponse(bean);
                            view.onReadSignServiceAssessItemFailure(msg);
                        }
