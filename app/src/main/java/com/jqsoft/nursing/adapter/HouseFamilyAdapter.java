 ...
 * 家庭成员
 */
public class HouseFamilyAdapter extends BaseQuickAdapterEx<FamilyMemberListBean, BaseViewHolder> {
    private Context context;

    public HouseFamilyAdapter(List<FamilyMemberListBean> data, Context context) {
        super(R.layout.item_servery_faimilymember, data);
    ...
    @Override
    protected void convert(final BaseViewHolder helper, final FamilyMemberListBean item) {
...
//        GlideUtils.loadImage(imageUrl, (ImageView) helper.getView(R.id.iv_title));
        helper.setText(R.id.username, Util.trimString(item.getSPEOPNAME()));
        helper.setText(R.id.relationship, Util.trimString(item.getSFAMILYGXNAME()));
        helper.setText(R.id.re_idcard, Util.trimString(item.getSIDCARD()));
