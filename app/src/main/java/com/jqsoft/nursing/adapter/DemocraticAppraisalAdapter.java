 ...
 * 签约服务评价适配器
 */
public class DemocraticAppraisalAdapter extends BaseQuickAdapterEx<DemocraticAppraisalBean, BaseViewHolder> {
    private Context context;

    public DemocraticAppraisalAdapter(List<DemocraticAppraisalBean> data, Context context) {
        super(R.layout.item_democraticappraisal, data);
    ...
    @Override
    protected void convert(final BaseViewHolder helper, final DemocraticAppraisalBean item) {
...
//        GlideUtils.loadImage(imageUrl, (ImageView) helper.getView(R.id.iv_title));
        helper.setText(R.id.responsname, Util.trimString(item.getPINGYIRENNAME()));
        helper.setText(R.id.householddata, Util.trimString(item.getPINGYIDATE()));
        helper.setText(R.id.householdresult, Util.trimString(item.getPINGYIRESULTDESC()));
//        helper.setText(R.id.re_sex, Util.trimString(item.getSEX()));
//        helper.setText(R.id.re_bithday, Util.trimString(item.getDDATE()));
