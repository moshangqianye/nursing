 ...
 * 签约服务评价适配器
 */
public class HouseFileAdapter extends BaseQuickAdapterEx<HouseHoldFileList, BaseViewHolder> {
    private Context context;
    private List<LocalMedia> selectMediax  =new ArrayList<>();
    private String golbIp;

    public String getGolbIp() {
        return golbIp;
    }

    public void setGolbIp(String golbIp) {
        this.golbIp = golbIp;
    }

    public HouseFileAdapter(List<HouseHoldFileList> data, Context context) {
        super(R.layout.item_servery_contentview_flie, data);
    ...
    @Override
    protected void convert(final BaseViewHolder helper, final HouseHoldFileList item) {
...
//        GlideUtils.loadImage(imageUrl, (ImageView) helper.getView(R.id.iv_title));
        helper.setText(R.id.filename, Util.trimString(item.getShowName()));
        helper.setText(R.id.filetype, Util.trimString(item.getFileTypeName()));
        helper.getView(R.id.button).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                selectMediax.clear();
                LocalMedia localMedia = new LocalMedia();
                localMedia.setPath(getGolbIp());
                localMedia.setUrl(item.getFilePath());
                selectMediax.add(localMedia);
                PictureConfig.getInstance().externalPicturePreview((Activity) context, 0, selectMediax);
            }
        });
