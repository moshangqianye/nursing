...
public class CustomRadioGroup extends LinearLayout {
    ...
    private int horizontalSpacing =10;//默认水平间距
    ...
    class RowView {
        ...
        public void addChidView(View view) {
            if (!lineViews.contains(view)) {
                ((SmoothRadioButton)view).setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if(listener!=null){
                            listener.OnText(((SmoothRadioButton)v).getText().toString().trim());
                        }
                    }
                });
                //更新高度和宽度
