...
import com.jqsoft.nursing.adapter.GroupRecyclerAdapter;
...
public class GroupItemDecoration<Group, Child> extends RecyclerView.ItemDecoration {
     ...
     */
    public void notifyDataSetChanged(GroupRecyclerAdapter adapter) {
        mGroup.clear();
        if (adapter == null) return;
        int key = 0;
        for (int i = 0; i < adapter.getGroupCount(); i++) {
            if (i == 0) {
                mGroup.put(isHasHeader ? 1 : 0, (Group) adapter.getGroup(i));
                key += adapter.getChildCount(i) + (isHasHeader ? 1 : 0);
                ;
            } else {
                mGroup.put(key, (Group) adapter.getGroup(i));
